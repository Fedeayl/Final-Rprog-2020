...
Por otra parte, el mayor diferencia a favor la obtuvo el `r T2$Ganador[which.max(as.numeric(as.vector(T2$Margen)))]`, con el `r max(as.numeric(as.vector(T2$Margen)))`% de ventaja en el departamento de `r T2$Depto[which.max(as.numeric(as.vector(T2$Margen)))]` , mientras que el margen más acotado se dio en `r T2$Depto[which.min(as.numeric(as.vector(T2$Margen)))]` donde el `r T2$Ganador[which.min(as.numeric(as.vector(T2$Margen)))]` se impuso con tan solo  el `r min(as.numeric(as.vector(T2$Margen)))`% de los votos.  
